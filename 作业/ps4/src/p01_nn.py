...
def backward_softmax(x, grad_outputs):
    ...
    # part L / part x_i = [softmax(x_i) - \sum_k softmax(x_k) ] * grad_outputs
    grad_x = softmax_x * grad_outputs - softmax_x * (softmax_x @ grad_outputs)
    return grad_x
    # *** END CODE HERE ***
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    # *** START CODE HERE ***
    # 每一项倍数为1，最后导数即是求和
    bias_grad = np.sum(output_grad, axis=(1, 2))
    ...
    input_channels, input_width, input_height = data.shape
    # 输出维度，输入维度， 高， 宽
    conv_c, _, conv_w, conv_h = conv_W.shape

    # 求解dW, dA
    for x in range(input_width - conv_w + 1):
        for y in range(input_height - conv_h + 1):
            for oc in range(conv_c):
                weight_grad[oc, :, :, :] += output_grad[oc, x, y] * data[:, x: (x + conv_w), y:(y + conv_h)]
                data_grad[:, x: (x + conv_w), y: (y + conv_h)] += output_grad[oc, x, y] * conv_W[oc, :, :, :]
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    for c in range(dc):
        for i in range(0, dw, pool_width):
            for j in range(0, dh, pool_height):
                t = data[c, i:(i + pool_width), j:(j + pool_height)]
                grads[c, i:(i + pool_width), j:(j + pool_height)][c, np.unravel_index(t.argmax(), t.shape)] = \
                    output_grad[c, i // pool_width, j // pool_height]
    return grads
    # *** END CODE HERE ***
...
def backward_linear(weights, bias, data, output_grad):
    ...
    # *** START CODE HERE ***
    w_grad = np.outer(data,output_grad)
    b_grad = output_grad
    d_grad = weights.dot(output_grad)
    return w_grad, b_grad, d_grad
    # *** END CODE HERE ***
