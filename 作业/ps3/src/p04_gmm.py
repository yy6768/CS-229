...
def main(is_semi_supervised, trial_num):
    ...
    for j in range(K):
        ...
        sigma[j] = np.cov(x_j, rowvar=False)
...
def run_em(x, phi, mu, sigma):
    ...
    for i in range(max_iter):
        ...
        for j in range(K):
            ...
            log_prob[:, j] = - np.log(LA.det(sigma[j])) / 2 - exponents / 2 + np.log(phi[j])  # 计算似然
...
def run_semi_supervised_em(x, x_tilde, z_tilde, phi, mu, sigma):
    ...
    for it in range(max_iter):
        ...
        for j in range(K):
            exponents = (x - mu[j]) @ LA.inv(sigma[j]) * (x - mu[j])
            exponents = exponents.sum(axis=1)
            log_prob[:, j] = - np.log(LA.det(sigma[j])) / 2 - exponents / 2 + np.log(phi[j])  # log p(x,z)
        w = np.exp(log_prob - logsumexp(log_prob).reshape(-1, 1))  # 指数计算防止爆炸

        # Check convergence
        # Stop when the absolute change in log-likelihood is < eps
        ll = (- num_total * dim / 2 * np.log(2 * np.pi)  # 2pi^(n/2)
              + np.sum(logsumexp(log_prob[:-n1]))  # unlabeled
              + np.sum(w1 * log_prob[-n1:]))  # labeled
        if np.abs(ll - prev_ll) < eps:
...
def plot_gmm_preds(x, z, with_supervision, plot_id):
    ...
    if with_supervision:
        file_name = f'p04_semisup_gmm{plot_id}.png'
    else:
        file_name = f'p04_unsup_gmm{plot_id}.png'
